...
from analysis.opportunity import analyze_opportunity, OpportunityEngine, ListingQualityDetector, MetadataDetector, MarketSignalsDetector, SellerMotivationDetector, AIDetector
...
def test_listing_quality_detector():
    ...
    # Test poor photos
    listing = {"title": "Good Title", "description": "Long enough description here to avoid that trigger.", "images": ["1"]}
    factors = detector.detect(listing)
    assert any(f.name == "few_photos" for f in factors)
    
    # Test generic title
    listing = {"title": "stuff", "description": "Long enough description here to avoid that trigger.", "images": ["1", "2"]}
    factors = detector.detect(listing)
...
def test_metadata_detector():
    ...
    listing = {"title": "Fender Stratocaster American Professional II", "description": "Serial number 12345"}
    factors = detector.detect(listing)
...
def test_market_signals_detector():
    ...
    # Test bundle
    listing = {"title": "Camera bundle with lenses", "description": "Everything must go"}
    factors = detector.detect(listing)
