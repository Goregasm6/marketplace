...
def get_categories() -> tuple[CategoryKnowledge, ...]:
    ...
    plugins = [plugin for plugin in registry.get_categories() if isinstance(plugin, CategoryKnowledge)]
...
def get_category(slug: str | None) -> CategoryKnowledge | None:
    ...
    return next((plugin for plugin in get_categories() if plugin.category == normalized), None)


def all_brands() -> tuple[str, ...]:
    return tuple(dict.fromkeys(brand for category in get_categories() for brand in category.brands))
...
def keyword_catalog() -> dict[str, tuple[str, ...]]:
    ...
    return {
        category.category: tuple(
            dict.fromkeys(
                (*category.keywords, *category.brands, *category.common_misspellings, *category.common_model_prefixes)
            )
        )
