...
def match_score(
    listing: ParsedListing, comparable: object
    ...
    if listing.recognized_brand and normalize_title(
        str(getattr(comparable, "brand", "") or "")
    ) == normalize_title(listing.recognized_brand):
        score += 0.25
    if listing.recognized_model and normalize_title(
        listing.recognized_model
    ) in normalize_title(title):
        score += 0.35
