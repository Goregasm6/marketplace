...
def calculate_confidence(*, match_scores: Sequence[float], prices: Sequence[float], recognized: bool) -> float:
    ...
    variation = pstdev(prices) / average_price if len(prices) > 1 and average_price else 1.0
    consistency_score = max(0.0, 1.0 - min(variation, 1.0))
    recognition_score = 1.0 if recognized else 0.45
    return round(min(1.0, 0.20 * sample_score + 0.40 * quality_score + 0.25 * consistency_score + 0.15 * recognition_score), 2)
