...
class OpportunityFactor:
    ...
    name: str
    score_impact: float  # 0 to 100, how much this factor contributes to the overall score
    reason: str
...
class MetadataDetector(OpportunityDetector):
    ...
    def detect(self, listing: dict[str, Any]) -> List[OpportunityFactor]:
        ...
        has_model = re.search(model_pattern, title) or re.search(model_pattern, description)
...
class SpecializationDetector(OpportunityDetector):
    ...
    def detect(self, listing: dict[str, Any]) -> List[OpportunityFactor]:
        ...
        if any(term in category for term in ["industrial", "commercial", "lab", "medical"]):
            factors.append(OpportunityFactor(
