...
def _copy_table(source: sqlite3.Connection, target: duckdb.DuckDBPyConnection, table: str) -> int:
    schema = source.execute(f"PRAGMA table_info({_quote(table)})").fetchall()
    definitions = ", ".join(f"{_quote(column[1])} {_duckdb_type(column[2])}" for column in schema)
...
def _column(columns: set[str], name: str, *, table: str = "l", type_name: str = "VARCHAR") -> str:
    return f"{table}.{_quote(name)}" if name in columns else f"NULL::{type_name}"


def _create_analytics_views(source: sqlite3.Connection, target: duckdb.DuckDBPyConnection) -> None:
    ...
    opportunity_columns = _columns(source, "opportunities") if "opportunities" in tables else set()

    seller_join = "LEFT JOIN sellers s ON l.seller_id = s.id" if "sellers" in tables and "seller_id" in listing_columns else ""
    search_join = "LEFT JOIN searches se ON l.search_id = se.id" if "searches" in tables and "search_id" in listing_columns else ""
    if not seller_join:
        seller_columns = set()
    if not search_join:
        search_columns = set()
    if "opportunities" in tables and "listing_id" in opportunity_columns:
        ...
        opportunity_join = ""
    opportunity_projection = "o.expected_profit, o.opportunity_confidence" if opportunity_join else "NULL::DOUBLE AS expected_profit, NULL::DOUBLE AS opportunity_confidence"
    ...
    def sync(self) -> SyncResult:
        ...
        with _operational_connection(self.operational_path) as source:
            ...
            try:
                ...
                # Views depend on copied tables, so remove them before replacing
                # the snapshot tables during a subsequent sync.
                target.execute("DROP VIEW IF EXISTS listing_facts")
                for table in _sqlite_tables(source):
                    target.execute(f"DROP TABLE IF EXISTS {_quote(table)}")
                    table_counts[table] = _copy_table(source, target, table)
                ...
                target.close()
        return SyncResult(self.warehouse_path, table_counts)

    def connect(self) -> duckdb.DuckDBPyConnection:
        """Return a read-only DuckDB connection for analytics queries."""
        if not self.warehouse_path.exists():
            raise FileNotFoundError(f"Analytics warehouse does not exist: {self.warehouse_path}. Run sync() first.")
...
def sync_operational_data(
    ...
    return Warehouse(warehouse_path=warehouse_path, operational_path=operational_path).sync()
